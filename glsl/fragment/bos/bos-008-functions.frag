...
float makeRectangle( vec2 size, vec2 current ) {
	...
	float f = 1.0;
	vec2 tmp = step( size, current );
    f *= tmp.x * tmp.y;

    tmp = step( size, 1.0 - current );
    f *= tmp.x * tmp.y;
...
void main(){
	...
	vec2 rot = vec2(cos(u_time),sin(u_time));

	accumulator = addShape( accumulator, makeRectangle( vec2(0.1, 0.3 ), st + vec2( -0.3 ) ) );
